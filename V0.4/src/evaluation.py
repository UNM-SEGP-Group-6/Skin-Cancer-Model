...
def predict_image(image_path, model, device, transform, dataset=None, metadata=None, class_names=None):
    """
    ...
    Parameters:
        ...
        dataset (SkinLesionDataset, optional): Training dataset instance used to encode metadata/labels.
        metadata (dict or torch.Tensor, optional): Dictionary of raw metadata OR pre-encoded tensor.
        class_names (list, optional): List of class names. If None, uses dataset or config defaults.
    Returns:
    ...
    if dataset is not None:
        ...
        # Standalone mode
        if metadata is None:
            # Fallback to zero tensor. Assumes metadata_dim=13 based on architecture
            meta_tensor = torch.zeros((1, 13), dtype=torch.float32).to(device)
        elif isinstance(metadata, torch.Tensor):
            meta_tensor = metadata.to(device)
            if meta_tensor.dim() == 1:
                meta_tensor = meta_tensor.unsqueeze(0)
        else:
            raise ValueError("When running standalone (dataset=None), 'metadata' must be a pre-encoded torch.Tensor or None.")
        
        if class_names is None:
            try:
                from src.config import CLASS_SCHEME
                # Reverse CLASS_SCHEME to get an ordered list of names
                class_names = [k for k, v in sorted(CLASS_SCHEME.items(), key=lambda item: item[1])]
            except ImportError:
                class_names = [f"Class_{i}" for i in range(model.classifier[-1].out_features)]
